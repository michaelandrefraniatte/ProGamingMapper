...
namespace DualShock42API
{
    /// <summary>
    /// Interaction logic for DualShock42 controllers.
    /// </summary>
    public class DualShock42
    {
        // IO parameters
        ...
        private DualShock42(IDevice underlyingDevice, int? writeBufferSize)
        {
        ...
        /// expected that you will only use the <see cref="DualShock42"/> instance passed as an argument to 
        ...
        public async void BeginPolling()
        {
            ...
            L2 = GetModeSwitch(data, 7).ToUnsignedFloat();
            R2 = GetModeSwitch(data, 8).ToUnsignedFloat();

            // Buttons
            btnBlock1 = GetModeSwitch(data, 4);
            btnBlock2 = GetModeSwitch(data, 5);
            btnBlock3 = GetModeSwitch(data, 6);
            ...
            // Multitouch
            Touchpad1 = ReadTouchpad(GetModeSwitch(data, 34, 4));
            ...
            Gyro = -ReadAccelAxes(
                GetModeSwitch(data, 12, 2),
                GetModeSwitch(data, 14, 2),
                GetModeSwitch(data, 16, 2)
            );
            Accelerometer = ReadAccelAxes(
                GetModeSwitch(data, 18, 2),
                GetModeSwitch(data, 20, 2),
                GetModeSwitch(data, 22, 2)
            );

            miscByte = GetModeSwitch(data, 29); // this contains various stuff, seems to have both audio and battery info
        ...
        /// <returns>A parsed <see cref="DualShock42Touch"/>.</returns>
        private static DualShock42Touch ReadTouchpad(byte[] bytes)
        {
        ...
        /// </summary>
        public static DualShock42Touch Touchpad1 { get; private set; }

        /// <summary>
        /// The second touch point.
        /// </summary>
        public static DualShock42Touch Touchpad2 { get; private set; }
        ...
        public static DualShock42 EnumerateControllers(string vendor_id, string product_id, string label_id, bool similar)
        {
            int similarcount = 0;
            foreach (ConnectedDeviceDefinition deviceDefinition in ListDevices(vendor_id, product_id, label_id))
            {
                if (!similar)
                {
                    ...
                    return new DualShock42(device, deviceDefinition.WriteBufferSize);
                }
                else
                {
                    if (similarcount == 1)
        ...
        /// <summary>
        /// A handler for a state polling IO event. The sender has the <see cref="DualShock42InputState"/>
        /// from the most recent poll, and can be used to update the next
        /// <see cref="DualShock42OutputState"/>.
        /// </summary>
        /// <param name="sender">The <see cref="DualShock42"/> instance that was just polled.</param>
        public delegate void StatePolledHandler(DualShock42 sender);
    ...
    /// </summary>
    internal static class DualShock42ByteConverterExtensions
    {
        /// <summary>
    ...
    /// <summary>
    /// One of the DualShock42's 2 touch points. The touchpad is 1920x1080, 0-indexed.
    /// </summary>
    public struct DualShock42Touch
    {
        /// <summary>
